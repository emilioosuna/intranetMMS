<div class="box box-info padding-1">
    <div class="box-body">
        ...
        <div class="form-group">
                    {{ Form::label('Tienda') }}
                     <select name="tienda_id" id="tienda_id" class="form-control select2 {{  $errors->has('tienda_id') ? ' is-invalid' : '' }}" >
                        ...
                        <option value="{{ $tienda->id }}" {{ ((!$ventasVendedore->id ? auth()->user()->tienda : $ventasVendedore->tienda_id) == $tienda->id ) ? "selected" : "" }}>{{ $tienda->tienda }}</option>
                        @endforeach
                    </select>
        ...
        <div class="form-group">
            {{ Form::label('fdesde') }}
            {{ Form::date('fdesde', $ventasVendedore->fdesde, ['class' => 'form-control' . ($errors->has('fdesde') ? ' is-invalid' : ''), 'placeholder' => 'Fdesde']) }}
            {!! $errors->first('fdesde', '<div class="invalid-feedback">:message</div>') !!}
        </div>
        <div class="form-group">
            {{ Form::label('fdhasta') }}
            {{ Form::date('fdhasta', $ventasVendedore->fdhasta, ['class' => 'form-control' . ($errors->has('fdhasta') ? ' is-invalid' : ''), 'placeholder' => 'Fdhasta']) }}
